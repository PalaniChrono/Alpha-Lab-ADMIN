...
@Component({
  selector: 'app-faq-lab-details',
  templateUrl: './faq-lab-details.component.html',
  styleUrls: ['./faq-lab-details.component.css']
})
export class FaqLabDetailsComponent implements OnInit {
  ...
  index() {
      this.loading = true;
      this.apiService.index('getAllFaqLab').subscribe(data => {
  ...
  store() {
      ...
      this.apiCall('store', 'FaqLab');
  }

  show(id) {
      this.apiService.show('FaqLab/' + id).subscribe((data) => {
          const value = data.data;
          this.bannerName = value.header;
          this.description = value.header_answer;
