...
export class QuestionComponent implements OnInit {
  ...
  index() {
      this.loading = true;
      this.apiService.index('getAllQuestion').subscribe(data => {
  ...
  store() {
      ...
      this.apiCall('store', 'Question');
  }

  show(id) {
      this.apiService.show('Question/' + id).subscribe((data) => {
          const value = data.data;
          this.bannerName = value.question;
          this.description = value.question_answer;
  ...
  update(id) {
      this.loadingBtn = true;
      this.apiCall('update', 'Question/' + id);
  }
  ...
  bannerSwitch(id) {
      this.apiCall('getData', 'QuestionSwitch/' + id);
  }

  apiCall(name, url, value: any = '') {
      value = name === 'store' || name === 'update' ? {
        question: this.bannerName,
          question_answer: this.description,
  ...
  swalCall(id) {
      Swal.fire({
          ...
          if (result.value) {
              this.apiCall('destroy', 'Question/' + id);
              Swal.fire(
                  'Deleted!',
